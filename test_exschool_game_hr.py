...
from exschool_game.workforce import smoothed_average_salary, workforce_plan
...
def test_promotion_timing_matches_two_round_rule() -> None:
    ...
    r1_payload["worker_salary"] = 5000
    r1_payload["engineer_salary"] = 8000
    ...
    r2_payload["worker_salary"] = 5000
    r2_payload["engineer_salary"] = 8000
    ...
    r3_payload["worker_salary"] = 5000
    r3_payload["engineer_salary"] = 8000
...
def test_salary_cut_quits_continue_after_recent_staff_are_exhausted() -> None:
    plan = workforce_plan(
        requested_total=100,
        requested_salary=1000.0,
        benchmark_average_salary=5000.0,
        previous_recent=1,
        previous_mature=2,
        previous_experienced=97,
    )

    assert plan["quits"] > plan["quits_inexperienced"]
    assert plan["quits_experienced"] > 0
    assert plan["working_experienced"] < 97
...
def test_round2_default_payload_uses_round2_decision_defaults_not_round1_workforce() -> None:
    ...
    assert state.last_worker_salary == 3333
    assert state.last_engineer_salary == 6666
    assert state.worker_avg_salary != 3333
    assert state.engineer_avg_salary != 6666
