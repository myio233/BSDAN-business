...
def test_high_intensity_r1_recommended_strategy_is_outcompeted_by_smart_opponents() -> None:
    simulator = get_simulator("high-intensity")
    for home_city in ["Shanghai", "Chengdu"]:
        ...
        payload["workers"] = int(simulator.round_contexts["r1"]["workers_actual"])
        payload["engineers"] = int(simulator.round_contexts["r1"]["engineers_actual"])
...
def test_high_intensity_round_one_default_payload_starts_without_existing_headcount() -> None:
    simulator = get_simulator("high-intensity")
...
def _extract_round_page_data(response_text: str) -> dict[str, object]:
...
def test_single_fixed_round_one_page_prefills_zero_headcount_delta() -> None:
    ...
    assert response.status_code == 200
    page_data = _extract_round_page_data(response.text)
    initial_payload = page_data["initialPayload"]
    assert page_data["currentWorkers"] == 0
    assert page_data["currentEngineers"] == 0
    assert initial_payload["workers"] == 0
    assert initial_payload["engineers"] == 0


def test_real_original_round_two_page_keeps_source_faithful_default_loan() -> None:
    client, user = _register_logged_in_client()
    _start_game(client, "/single-real/start", company_name="Source Loan Co")

    _submit_active_round(client, user)
    round_two_html = _advance_active_round(client, user)
    page_data = _extract_round_page_data(round_two_html)

    assert page_data["initialPayload"]["loan_delta"] == 10_000_000.0


def test_real_original_page_defaults_replay_source_financials_all_rounds() -> None:
    client, user = _register_logged_in_client()
    _start_game(client, "/single-real/start", company_name="Page Defaults Co")
    response = client.get("/game").text
    expected = {
        "r1": (58_843_246.0, 4_894_719.0, 72_022_320.0),
        "r2": (164_839_081.0, 5_046_197.0, 194_878_304.0),
        "r3": (488_113_013.0, 15_512_526.0, 599_418_807.0),
        "r4": (1_054_450_611.0, 26_303_413.0, 1_317_640_000.0),
    }

    for round_id, numbers in expected.items():
        page_data = _extract_round_page_data(response)
        payload = page_data["initialPayload"]
        csrf_token = _fetch_csrf_token(client, "/game")
        submit_response = client.post(
            "/game/submit",
            data=_round_form_data(payload, csrf_token=csrf_token, game_id=str(page_data["gameId"])),
            follow_redirects=False,
        )
        assert submit_response.status_code == 200
        active_session = user_game_store.get_active_game_session(user["client_id"])
        assert active_session is not None
        report = active_session["latest_report_detail"]
        assert report["round_id"] == round_id
        assert report["ending_cash"] == numbers[0]
        assert report["ending_debt"] == numbers[1]
        assert report["key_metrics"]["销售收入"] == numbers[2]
        if round_id != "r4":
            response = _advance_active_round(client, user)


def test_mode_page_exposes_single_player_entrypoints() -> None:
    client, _user = _register_logged_in_client()
    ...
    assert response.status_code == 200
    assert "真实原版竞争" in response.text
    assert "/single-real/setup" in response.text
    assert "开始真实原版竞争" in response.text
    assert "高强度竞争" in response.text
    assert "/single-fixed/setup" in response.text
    assert "开始高强度竞争" in response.text


def test_single_fixed_setup_route_uses_high_intensity() -> None:
    client, _user = _register_logged_in_client()

    response = client.get("/single-fixed/setup")

    assert response.status_code == 200
    assert 'data-testid="mode-chip">高强度竞争</span>' in response.text
    assert 'action="http://testserver/single-fixed/start"' in response.text
...
def test_multiple_active_games_can_be_selected_from_home() -> None:
    ...
    assert "真实原版竞争" in alpha_response.text
