...
def check_scripts() -> list[CheckResult]:
    ...
    if LAUNCH_SCRIPT.exists():
        ...
        results.append(CheckResult("WARN", "launch-script", f"optional script missing: {LAUNCH_SCRIPT.name}"))
    if REVIEW_SCRIPT.exists():
        results.append(CheckResult("PASS", "review-script", f"found {REVIEW_SCRIPT.name}"))
    else:
        results.append(CheckResult("WARN", "review-script", f"optional script missing: {REVIEW_SCRIPT.name}"))
...
def check_canonical_baseline() -> CheckResult:
    ...
    if exschool <= 0.95:
        return CheckResult("FAIL", "baseline", f"Exschool={exschool:.6f}, OBOS={obos:.6f}")
    level = "PASS" if obos > 0.40 else "WARN"
...
def check_real_original_coverage() -> CheckResult:
    ...
    observed = int(info.get("observed_team_count") or 0)
    expected = int(info.get("expected_team_count") or 0)
    if bool(info.get("coverage_complete", False)) or (expected > 0 and observed >= expected):
