...
- 报表抽取：把 `exschool/` 里的长图和原始 market report workbook 整理成结构化输出
- 决策反推：把结构化 market report 反推出固定对手输入
- 模型校验：让模拟器基于当前结构化数据重新训练，并跑行为回归测试

#### 当前依赖和 caveat

- Python 依赖主要在 `requirements-exschool-game.txt`
- 真实报表抽取额外依赖 `tesseract`，它不在上面的 requirements 里
- `skills/image-report-html-table/scripts/export_market_report_tables.py` 和 `obos/reconstruct_exschool_decisions.py` 现在按脚本相对路径解析 repo 根目录，不再依赖固定 checkout 路径
- 但要重跑抽取，当前环境仍需同时具备：
  - `.venv` 里的 `pytesseract`
  - 系统可执行的 `tesseract`
...
- `outputs/exschool_market_report_exports/html/*.html`
- `outputs/exschool_market_report_exports/combined_xlsx/*.xlsx`
- `outputs/exschool_market_report_exports/structured_xlsx/report*_market_reports*.xlsx`
- `outputs/exschool_market_report_exports/validation_report.xlsx`
...
- `outputs/exschool_inferred_decisions/all_visible_market_decisions.xlsx`
- `outputs/exschool_inferred_decisions/all_round_reconstruction_summary.xlsx`
- `outputs/exschool_inferred_decisions/all_companies_numeric_decisions.xlsx`
- `outputs/exschool_inferred_decisions/C##_反推决策模板.xlsx`
- `outputs/exschool_inferred_decisions/C##_纯决策数值.xlsx`
- `outputs/exschool_inferred_decisions/ASSUMPTIONS.txt`

第三步：让模拟器基于当前数据重训并校验

- `ExschoolSimulator()` 初始化时会读取 market reports、Team13 实际 round workbook、fixed decisions
- 然后在进程内重训 CPI 模型和 share 模型
- 这里没有单独持久化的模型文件，当前逻辑是“数据一变，进程重启后重新训练”

#### 必查 validation

- `outputs/exschool_market_report_exports/validation_report.xlsx` 里的 `ok` 不能有 `False`
- OCR 检出的 round 必须和预期 `r1` 到 `r4` 一致
- OCR 检出的城市标题必须和源 workbook sheet 名一致
- `outputs/exschool_inferred_decisions/all_round_reconstruction_summary.xlsx` 必须存在
- `outputs/exschool_inferred_decisions/ASSUMPTIONS.txt` 必须一起看，因为工资、人员、贷款、研发、库存都不是 market report 里唯一可识别的真值
- `outputs/exschool_inferred_decisions/all_companies_numeric_decisions_real_original_fixed.xlsx` 的 `validation` sheet 要区分两类问题：
  - 聚合正确性问题：`input_file_count` / `combined_row_count` / `source_team_ids`
  - 源数据覆盖问题：例如当前 source roster 覆盖 23 队并缺少 team `11`
- 不要把“脚本跑完了”当成通过，这条链路真正的验收是 workbook 级验证

#### 自动 bootstrap 的边界

`exschool_game/data_loader.py` 会在缺少结构化 market reports 或 fixed decisions 时尝试自动调用抽取和反推脚本。

这只是兜底，不应该替代手动检查 `validation_report.xlsx` 和 reconstruction 输出。
...
```bash
python3 -m pytest -q test_exschool_game_hr.py test_market_allocation.py test_inventory.py test_finance.py test_report_payload.py test_campaign_support.py test_modeling.py
```

这组测试主要覆盖 simulator 的行为回归，不覆盖：

- OCR / HTML / Excel 导出脚本
- `obos/reconstruct_exschool_decisions.py` 的 workbook 反推链路
- `tesseract` 是否已安装

另外还有两个环境 caveat：

- 这份仓库当前不能假设系统里一定有 `python` 命令；如果没有，就用 `python3` 或项目自己的虚拟环境解释器
- 这份仓库也不能假设系统里已经装好了 `pytest` / `pytesseract`；先装 `requirements-exschool-game.txt` 或进入已有虚拟环境再跑

所以“pytest 通过”不等于“真实报表抽取链路通过”。数据链路仍然要看上面的 workbook 验证产物。
