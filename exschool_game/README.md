...
- [campaign_support.py](campaign_support.py)
...
- [market_allocation.py](market_allocation.py)
  市场销量分配规则：
  货量按权重分配
  主场优先
  缺口吸收
...
- [report_payload.py](report_payload.py)
  报表 payload 组装层：
  市场结果
  peer market tables
...
- [export_report_html.py](export_report_html.py)
...
- [scripts/screenshot_html.py](scripts/screenshot_html.py)
  把生成好的 HTML 转成图片。
...
用户在 [round.html](templates/round.html) 里填写：
...
[campaign_support.py](campaign_support.py) 和 [engine.py](engine.py) 会把前端 payload 变成 `SimulationInput`：
...
[engine.py](engine.py) 中的 `_apply_cash_break_to_decision()` 会先做一轮“现金是否足够”的裁剪。
...
[report_payload.py](report_payload.py) 会把所有结果拼成最终 `report`：
...
- [engine.py](engine.py) 的 `_simulate_market()`
- [modeling.py](modeling.py)
...
这些特征定义在 [modeling.py](modeling.py)：
...
虽然已经拆出很多模块，但 [engine.py](engine.py) 仍然负责：
