...
def build_report_notes(mode: str, current_team_id: str = "13") -> list[str]:
    normalized_team_id = str(current_team_id).strip()
    bare_team_id = normalized_team_id[1:] if normalized_team_id.upper().startswith("C") else normalized_team_id
    controlled_team_label = f"当前队伍 C{bare_team_id}"
    input_label = "Team 13 使用当前输入" if bare_team_id == "13" else f"C{bare_team_id} 使用当前输入"
    notes = [
        f"所选轮次中，所有队伍都会按统一规则从各自决策出发参与同一轮竞争；{input_label}，当前展示的是 {controlled_team_label} 的真实结算结果。",
