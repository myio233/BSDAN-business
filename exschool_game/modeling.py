...
def build_cpi_to_share_feature_matrix(
    ...
    *,
    ...
    population = df["population"].fillna(0.0).clip(lower=0.0) if "population" in df.columns else pd.Series(0.0, index=df.index)
    penetration = df["penetration"].fillna(0.0).clip(lower=0.0) if "penetration" in df.columns else pd.Series(0.0, index=df.index)
    out["population_log"] = np.log1p(population)
    out["penetration_raw"] = penetration
    out["predicted_cpi_x_penetration"] = out["predicted_cpi"] * out["penetration_raw"]
    prev_marketshare = (
        df["prev_marketshare_clean"].fillna(0.0).clip(lower=0.0, upper=1.0)
        if "prev_marketshare_clean" in df.columns
        else pd.Series(0.0, index=df.index)
    )
    round_number = df["round"].astype(str).str.extract(r"(\d+)", expand=False).fillna("0").astype(float)
    market_name = df["market"].astype(str).str.casefold() if "market" in df.columns else pd.Series("", index=df.index)
    out["prev_marketshare_raw"] = prev_marketshare
    out["late_round_gate"] = (round_number >= 3.0).astype(float)
    ...
    out["late_hangzhou_r4_gate"] = ((round_number >= 4.0).astype(float)) * out["is_hangzhou_market"]
    out["late_hangzhou_prev_share"] = out["late_hangzhou_gate"] * out["prev_marketshare_raw"]
...
def predict_share_from_cpi_model(
    ...
    if calibrator:
        gate = share_X.get(str(calibrator.get("gate_column", "")), pd.Series(0.0, index=share_X.index)).to_numpy(dtype=float) > 0.0
        prev_share = share_X.get(
            str(calibrator.get("prev_share_column", "")),
            pd.Series(0.0, index=share_X.index),
        ...
        comparison_ceiling = np.maximum(prev_share * ceiling_multiplier, 0.0)
        adjustment = np.clip(prev_share * prev_share_slope, 0.0, max_adjustment)
        applies = gate & (base_share >= cpi_threshold) & (prev_share >= prev_share_threshold) & (share_pred < comparison_ceiling)
        residual = np.where(applies, adjustment, 0.0)
        share_pred = np.clip(share_pred + residual, 0.0, 1.0)
        delta_pred = np.clip(delta_pred + residual, -1.0, 1.0)
