...
def parse_fixed_team_decisions(mode: str | None = None) -> pd.DataFrame:
    ...
    if normalized_mode == "high-intensity":
        df = _apply_high_intensity_opponent_defaults(df)
    _log_fixed_decision_provenance(normalized_mode, source_dir=workbook.parent, df=df)
    return df


def _apply_high_intensity_opponent_defaults(df: pd.DataFrame) -> pd.DataFrame:
    """Keep Team 13 historical defaults while making fixed opponents meaningfully competitive."""
    if df.empty:
        return df
    boosted = df.copy()
    active_markets_by_round = {
    ...
    products_by_round = {"r1": 1_200, "r2": 1_800, "r3": 2_500, "r4": 3_300}
    management_by_round = {"r1": 1_200_000.0, "r2": 1_600_000.0, "r3": 2_200_000.0, "r4": 3_000_000.0}
    quality_by_round = {"r1": 100_000.0, "r2": 150_000.0, "r3": 220_000.0, "r4": 300_000.0}
    marketing_by_round = {"r1": 320_000.0, "r2": 420_000.0, "r3": 520_000.0, "r4": 600_000.0}
    price_by_round = {"r1": 20_000.0, "r2": 19_500.0, "r3": 19_000.0, "r4": 18_500.0}
    for idx, row in boosted.iterrows():
        if str(row.get("team")) == "13":
            continue
        round_id = str(row.get("round_id"))
        products = int(products_by_round.get(round_id, 900))
        boosted.at[idx, "loan_delta"] = 5_000_000.0
        boosted.at[idx, "products_planned"] = products
        boosted.at[idx, "workers"] = max(int(round(products * 7.0 / 22.0)), 1)
        boosted.at[idx, "engineers"] = max(int(round(products / 8.0)), 1)
        boosted.at[idx, "worker_salary"] = 3_200.0
        boosted.at[idx, "engineer_salary"] = 6_100.0
        boosted.at[idx, "management_investment"] = management_by_round.get(round_id, 1_800_000.0)
        boosted.at[idx, "quality_investment"] = quality_by_round.get(round_id, 120_000.0)
        boosted.at[idx, "research_investment"] = 0.0
        active_markets = set(active_markets_by_round.get(round_id, active_markets_by_round["r1"]))
        for market in ["Shanghai", "Chengdu", "Wuhan", "Wuxi", "Ningbo"]:
            prefix = market.lower()
            is_active = market in active_markets
            boosted.at[idx, f"{prefix}_selected"] = 1 if is_active else 0
            boosted.at[idx, f"{prefix}_agent_change"] = 1 if is_active else 0
            boosted.at[idx, f"{prefix}_agents_after"] = 1 if is_active else 0
            boosted.at[idx, f"{prefix}_marketing_investment"] = marketing_by_round.get(round_id, 420_000.0) if is_active else 0.0
            boosted.at[idx, f"{prefix}_price"] = price_by_round.get(round_id, 18_000.0) if is_active else 0.0
    return boosted
