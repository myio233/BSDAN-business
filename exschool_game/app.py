...
def _state_from_session(raw: object) -> CampaignState | None:
    ...
    return CampaignState(
        ...
        last_worker_salary=float(raw.get("last_worker_salary", raw["worker_salary"])),
        last_engineer_salary=float(raw.get("last_engineer_salary", raw["engineer_salary"])),
    )
...
def _state_to_session(state: CampaignState | None) -> dict[str, object] | None:
    ...
    return {
        ...
        "last_worker_salary": state.last_worker_salary if state.last_worker_salary is not None else state.worker_salary,
        "last_engineer_salary": state.last_engineer_salary if state.last_engineer_salary is not None else state.engineer_salary,
...
def _build_round_page_context(
    ...
    *,
    ...
    if mode_key == "real-original" and not session.get("multiplayer_room_id"):
        round_context["payload_loan_delta"] = payload_loan_delta
        state_for_payload = state or simulator._campaign_state_from_context(round_context)
