...
from .report_payload import assemble_simulation_report, build_report_notes
...
class ExschoolSimulator:
    def __init__(self, single_player_mode: str = "real-original") -> None:
    ...
    def _initial_company_state(
        ...
        *,
        ...
        if self.single_player_mode == "real-original" and preserve_real_original_round1:
            ...
            if summary_row is not None and not decision_row.empty:
                ...
                return CampaignState(
                    ...
                    last_round_id=None,
                    last_worker_salary=float(summary_row.get("worker_salary_est", home_market.get("initial_worker_salary", initial_worker_avg)) or 0.0),
                    last_engineer_salary=float(summary_row.get("engineer_salary_est", home_market.get("initial_engineer_salary", initial_engineer_avg)) or 0.0),
                )
        ...
        return CampaignState(
            ...
            accumulated_research_investment=0.0,
            last_round_id=None,
            last_worker_salary=float(home_market.get("initial_worker_salary", initial_worker_avg)),
            last_engineer_salary=float(home_market.get("initial_engineer_salary", initial_engineer_avg)),
        )
        ...
        *,
        ...
        context = {
            **base,
            ...
            "worker_salary_actual": float(effective_state.last_worker_salary if effective_state.last_worker_salary is not None else effective_state.worker_salary),
            "engineer_salary_actual": float(effective_state.last_engineer_salary if effective_state.last_engineer_salary is not None else effective_state.engineer_salary),
    ...
    def _campaign_state_from_context(self, context: dict[str, Any]) -> CampaignState:
        return CampaignState(
            ...
            last_worker_salary=float(context.get("payload_worker_salary", context.get("worker_salary_actual", 0.0)) or 0.0),
            last_engineer_salary=float(context.get("payload_engineer_salary", context.get("engineer_salary_actual", 0.0)) or 0.0),
        )
        ...
        *,
        ...
        if self.single_player_mode == "real-original":
            ...
            if fixed is not None:
                ...
                default_worker_salary = (
                    float(state.last_worker_salary if state.last_worker_salary is not None else state.worker_salary)
                    if state is not None
                    else float(fixed.worker_salary)
                )
                default_engineer_salary = (
                    float(state.last_engineer_salary if state.last_engineer_salary is not None else state.engineer_salary)
                    if state is not None
                    else float(fixed.engineer_salary)
                )
                return {
                    "round_id": round_id,
    ...
    def _context_with_campaign_state(
        ...
        if state is None:
            ...
            context = {
                **base,
                ...
                "payload_workers": (
                    int(base.get("workers_actual", 0) or 0)
                    if self.single_player_mode == "real-original" or str(round_id).lower() != "r1"
                    else 0
                ),
                "payload_engineers": (
                    int(base.get("engineers_actual", 0) or 0)
                    if self.single_player_mode == "real-original" or str(round_id).lower() != "r1"
                    else 0
                ),
        ...
        context = {
            **base,
            ...
            "worker_salary_actual": state.last_worker_salary if state.last_worker_salary is not None else state.worker_salary,
            "engineer_salary_actual": state.last_engineer_salary if state.last_engineer_salary is not None else state.engineer_salary,
            "market_defaults": market_defaults,
            ...
            "payload_engineers": (
                int(base.get("engineers_actual", 0) or 0)
                if self.single_player_mode == "real-original" or str(round_id).lower() != "r1"
                else int(state.engineers)
            ),
            "payload_worker_salary": float(state.last_worker_salary if state.last_worker_salary is not None else state.worker_salary),
            "payload_engineer_salary": float(state.last_engineer_salary if state.last_engineer_salary is not None else state.engineer_salary),
            "payload_management_investment": float(base.get("management_investment_actual", 0.0) or 0.0),
            "payload_quality_investment": float(base.get("quality_investment_actual", 0.0) or 0.0),
            "payload_research_investment": float(base.get("research_investment", 0.0) or 0.0),
            "payload_products_planned": int(base.get("products_produced_actual", 0) or 0),
    ...
    def _apply_cash_break_to_decision(
        ...
        *,
        ...
        return SimulationInput(
            ...
            products_planned=production_snapshot.target_products,
    ...
    def _fixed_decision_for_team(self, round_id: str, team: str) -> SimulationInput | None:
        ...
        expected_subscription_count = int(
            round(float(self.round_contexts.get(round_id, {}).get("market_report_cost", 0.0) or 0.0) / MARKET_REPORT_SUBSCRIPTION_COST)
        )
        use_explicit_selection = has_explicit_selection and (
            expected_subscription_count <= 0 or len(explicit_selected_markets) == expected_subscription_count
        )

        market_decisions: dict[str, MarketDecision] = {}
        def numeric_value(column: str, default: float = 0.0) -> float:
            value = row.get(column, default)
            if pd.isna(value):
                return float(default)
            return float(value)

        def integer_value(column: str, default: int = 0) -> int:
            return int(round(numeric_value(column, float(default))))

        for market in self.key_data["markets"].keys():
            prefix = market.lower()
            market_decisions[market] = MarketDecision(
                agent_change=integer_value(f"{prefix}_agent_change"),
                marketing_investment=numeric_value(f"{prefix}_marketing_investment"),
                price=numeric_value(f"{prefix}_price"),
        ...
        return SimulationInput(
            round_id=round_id,
            loan_delta=numeric_value("loan_delta"),
            workers=integer_value("workers"),
            engineers=integer_value("engineers"),
            worker_salary=numeric_value("worker_salary"),
            engineer_salary=numeric_value("engineer_salary"),
            management_investment=numeric_value("management_investment"),
            quality_investment=numeric_value("quality_investment"),
            research_investment=numeric_value("research_investment"),
            products_planned=integer_value("products_planned"),
            market_decisions=market_decisions,
        )
    ...
    def _financial_outcome_for_team(
        ...
        competition_support_income = 0.0
        if self.single_player_mode == "high-intensity" and str(team) != TEAM_ID and net_profit < 0.0:
            competition_support_income = abs(net_profit)
            ending_cash += competition_support_income
            total_assets = ending_cash
            net_assets = total_assets - ending_debt
            net_profit = 0.0
        return {
            ...
            "competition_support_income": competition_support_income,
            "worker_plan": worker_plan,
            "engineer_plan": engineer_plan,
        ...
        standings = [
            {
                ...
                "net_profit": float(outcome["net_profit"]),
                "sales_revenue": float(outcome["revenue"]),
                "competition_support_income": float(outcome.get("competition_support_income", 0.0)),
            }
    ...
    def _state_from_fixed_summary_round(self, round_id: str, team: str, benchmark_worker_avg: float, benchmark_engineer_avg: float) -> CampaignState | None:
        ...
        return CampaignState(
            ...
            last_worker_salary=float(summary_row.get("worker_salary_est", benchmark_worker_avg) or benchmark_worker_avg),
            last_engineer_salary=float(summary_row.get("engineer_salary_est", benchmark_engineer_avg) or benchmark_engineer_avg),
        )
    ...
    def _replay_real_original_report(
        ...
        *,
        ...
        workers_est = int(summary_row.get("workers_est", 0) or 0)
        engineers_est = int(summary_row.get("engineers_est", 0) or 0)
        worker_salary_est = float(summary_row.get("worker_salary_est", context.get("worker_salary_actual", 0.0)) or 0.0)
        engineer_salary_est = float(summary_row.get("engineer_salary_est", context.get("engineer_salary_actual", 0.0)) or 0.0)
        worker_avg_salary = float(context.get("worker_avg_salary_prev", worker_salary_est) or worker_salary_est)
        engineer_avg_salary = float(context.get("engineer_avg_salary_prev", engineer_salary_est) or engineer_salary_est)
        ...
        products_planned = int(summary_row.get("products_planned_est", 0) or 0)
        sales_units = float(summary_row.get("sales_units_source", 0.0) or 0.0)
        management_investment = float(summary_row.get("management_investment_est", 0.0) or 0.0)
        quality_investment = float(summary_row.get("quality_investment_est", 0.0) or 0.0)
        research_investment = float(summary_row.get("research_investment_est", 0.0) or 0.0)
        report = {
            "round_id": round_id,
            ...
            "finance_rows": [
                ...
                ("质量投入", -quality_investment, None, None, None),
            ...
            "hr_summary": [
                {"category": "工人", "working": workers_est, "salary": worker_salary_est},
                {"category": "工程师", "working": engineers_est, "salary": engineer_salary_est},
            ],
            "hr_detail": [
                {
                    "category": "Workers",
                    "previous": 0,
                    "previous_experienced": 0,
                    "added": workers_est,
                    "laid_off": 0,
                    "quits": 0,
                    "promotion_ready": 0,
                    ...
                    "experienced": 0,
                    "working": workers_est,
                    "working_total": workers_est,
                    "salary": worker_salary_est,
                    "avg": worker_avg_salary,
                    "salary_ratio": 1.0,
                ...
                },
                {
                    ...
                    "promoted_this_round": 0,
                    "experienced": 0,
                    "working": engineers_est,
                    "working_total": engineers_est,
                    "salary": engineer_salary_est,
                    "avg": engineer_avg_salary,
                    "salary_ratio": 1.0,
                    "productivity_multiplier": 1.0,
                },
            ],
            "management_summary": {
                "planned_investment": management_investment,
                "investment": management_investment,
                "index": management_index,
            },
            "production_summary": {
                "计划生产": products_planned,
                "计划生产数量": products_planned,
                "实际销量": sales_units,
                "实际售出数量": sales_units,
                "库存剩余成品": 0.0,
                "库存剩余零件": 0.0,
                "所需零件数": float(products_planned) * 7.0,
                "零件最大产能": 0.0,
                "成品最大产能": 0.0,
                "零件工资倍率": 1.0,
                "成品工资倍率": 1.0,
                "旧成品": 0.0,
                "新成品": products_planned,
                "质量投资": quality_investment,
                "产品质量指数": quality_index,
                "管理指数": management_index,
            },
            "production_overview": [],
            "production_details": [],
            "storage_summary": [
                {
                    ...
                    "capacity_after": 0.0,
                    "increment": 0.0,
                    "unit_price": float(context.get("component_storage_unit_cost", self.component_storage_unit_cost)),
                    "storage_cost": 0.0,
                },
                {
                    "item": "Products",
                    ...
                    "capacity_after": 0.0,
                    "increment": 0.0,
                    "unit_price": float(context.get("product_storage_unit_cost", self.product_storage_unit_cost)),
                    "storage_cost": 0.0,
                },
            ],
            "research_summary": {
            ...
            "notes": [
                "当前为 real-original 默认决策 replay：财报、排名、市场报表来自原始工作簿，不经过预测模型重算。",
                *build_report_notes(mode, str(context.get("team_id", TEAM_ID) or TEAM_ID)),
            ],
            ...
            "worker_plan": {
                "working": workers_est,
                "average_salary": worker_avg_salary,
                "next_recent": 0,
                "next_mature": 0,
                "next_experienced": workers_est,
            },
            "engineer_plan": {
                "working": engineers_est,
                "average_salary": engineer_avg_salary,
                "next_recent": 0,
                "next_mature": 0,
                "next_experienced": engineers_est,
            },
        ...
        *,
        ...
        for team in self.team_ids:
            ...
            next_states[team] = CampaignState(
                ...
                last_worker_salary=next_state.last_worker_salary,
                last_engineer_salary=next_state.last_engineer_salary,
            )
        return report, next_states
...
@lru_cache(maxsize=4)
def get_simulator(single_player_mode: str = "real-original") -> ExschoolSimulator:
