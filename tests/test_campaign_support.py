...
def test_next_campaign_state_carries_inventory_and_patents() -> None:
    state = next_campaign_state(
        report={
            ...
            "worker_plan": {"working": 11, "average_salary": 2800.0, "next_recent": 1, "next_mature": 2, "next_experienced": 8},
            "engineer_plan": {"working": 9, "average_salary": 4800.0, "next_recent": 1, "next_mature": 1, "next_experienced": 7},
        },
        decision=build_simulation_input(
            visible_markets={"Shanghai"},
            round_id="r1",
    ...
    assert state.worker_salary == 3000
    assert state.engineer_salary == 5000
    assert state.worker_avg_salary == 2800.0
    assert state.engineer_avg_salary == 4800.0
